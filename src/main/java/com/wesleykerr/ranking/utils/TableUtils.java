...
public class TableUtils {
    ...
    /**
     * Grab the value from the matrix. If non-existent, then return a default
     * value.
     * 
     * @param matrix
     * @param row
     ...
     * @return
     */
    public static <T extends Comparable<T>> double get(
            Table<T, T, Double> matrix, T row, T col) {
        Double value = matrix.get(row, col);
        return value == null ? 0.0 : value;
    }

    /**
     * compute the cosine distance for each item in the matrix.
     * 
     * @param matrix
     */
    public static <T extends Comparable<T>> Table<T, T, Double> computeCosine(
            Table<T, T, Double> table) {
        ...
        for (T row : table.rowKeySet()) {
            for (T col : table.columnKeySet()) {
                ...
                double cosSim = bothCount
                        / (Math.sqrt(rowCount) * Math.sqrt(colCount));
     ...
     * @return
     */
    public static <T extends Comparable<T>> Table<T, T, Double> rowNormalize(
            Table<T, T, Double> table) {
    ...
    /**
     * Merge the second ItemItemMatrix into the first. Note that this is a
     * mutable operation. We are altering m1.
     * 
     * @param t1
     * @param t2
     */
    public static <T extends Comparable<T>> void mergeInto(
            Table<T, T, Double> t1, Table<T, T, Double> t2) {
        for (Table.Cell<T, T, Double> cell : t2.cellSet()) {
            double oldValue = get(t1, cell.getRowKey(), cell.getColumnKey());
            t1.put(cell.getRowKey(), cell.getColumnKey(),
     ...
     * @param output
     */
    public static <T extends Comparable<T>> void writeCSVMatrix(
            Table<T, T, Double> t1, Writer out) throws Exception {
